...
//	log.Fatal(http.ListenAndServe("localhost:4444", nil))//for use behind reverse proxy, make sure to send X-Real-IP header.
}

func handler(w http.ResponseWriter, r *http.Request) {
	ip := ""
	if r.Header.Get("X-Real-IP") != "" {//autodetect if behind reverse proxy
...
func commandhandler(formMessage string, ip string) int{
	noun := ""
	if len(formMessage) > 6 {
		if formMessage[0:6] == "/motd "{
			noun = strings.TrimPrefix(formMessage, "/motd ")
			formMessage = "/motd"
		}
	}
	if len(formMessage) > 5 {
		if formMessage[0:5] == "/ban "{
