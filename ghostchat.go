...
//	log.Fatal(http.ListenAndServe("localhost:4444", nil))//For use behind reverse proxy, make sure to send X-Real-IP header. Disable above line to use this one, also enable line 59 and disable line 60.
}

func handler(w http.ResponseWriter, r *http.Request) {
	//ip := r.Header.Get("X-Real-IP") //Make sure your reverse proxy is configured so header X-Real-IP cannot be spoofed. Also see comments on line 54
